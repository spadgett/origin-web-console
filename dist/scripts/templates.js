angular.module('openshiftConsoleTemplates', []).run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('views/new-overview.html',
    ...
    "<div class=\"search-pf-input-group\">\n" +
    "<label for=\"name-filter\" class=\"sr-only\">Filter by name</label>\n" +
    "<input type=\"text\" class=\"form-control\" ng-model=\"overview.filterText\" placeholder=\"Filter by name\" autocorrect=\"off\" autocapitalize=\"off\" spellcheck=\"false\">\n" +
    "<button type=\"button\" class=\"clear\" aria-hidden=\"true\" ng-if=\"overview.filterText\" ng-click=\"overview.filterText = ''\">\n" +
    "<span class=\"pficon pficon-close\"></span>\n" +
    "</button>\n" +
    "</div>\n" +
    "</div>\n" +
    ...
    "\n" +
    "<div ng-if=\"!(overview.deploymentConfigsByPipeline | hashSize)\" class=\"empty-state-message text-center\">\n" +
    "<h2>No pipelines.</h2>\n" +
    "<div ng-if=\"project.metadata.name | canIAddToProject\">\n" +
    "<p>\n" +
    ...
    "</div>\n" +
    "</div>\n" +
    "<div ng-if=\"overview.deploymentConfigsByPipeline | hashSize\">\n" +
    "<div ng-repeat=\"(pipelineName, deploymentConfigNames) in overview.deploymentConfigsByPipeline\" class=\"mar-bottom-lg\">\n" +
    "<h2>\n" +
    ...
    "No pipeline runs.\n" +
    "</div>\n" +
    "<div ng-if=\"overview.recentPipelinesByBC[pipelineName] | hashSize\" class=\"build-pipelines\">\n" +
    "<div ng-repeat=\"pipeline in overview.recentPipelinesByBC[pipelineName] | orderObjectsByDate : true track by (pipeline | uid)\" class=\"row build-pipeline-wrapper animate-repeat animate-slide\">\n" +
    "<div class=\"col-sm-12\">\n" +
    "<build-pipeline build=\"pipeline\" build-config-name-on-expanded=\"true\"></build-pipeline>\n" +
    "</div>\n" +
    "</div>\n" +
    "</div>\n" +
    "<div ng-if=\"deploymentConfigNames.length\" class=\"list-pf\">\n" +
    "<overview-list-row ng-repeat=\"dcName in deploymentConfigNames\" api-object=\"overview.deploymentConfigs[dcName]\" current=\"overview.currentByDeploymentConfig[dcName]\" previous=\"overview.getPreviousReplicationController(deploymentConfig)\" state=\"overview.state\" hide-pipelines=\"true\">\n" +
    "</overview-list-row>\n" +
    "</div>\n" +
    "</div>\n" +
    "</div>\n" +
  ...
  $templateCache.put('views/overview/_builds.html',
    ...
    "<span ng-if=\"build.status.startTimestamp && build.status.phase !== 'Complete' && ('builds/log' | canI : 'get')\" class=\"small pull-right\">\n" +
    "<a ng-if=\"!!['New', 'Pending'].indexOf(build.status.phase) && (build | buildLogURL)\" ng-href=\"{{build | buildLogURL}}\">View <span ng-if=\"!overviewBuilds.hideLog\">Full</span> Log</a>\n" +
    "</span>\n" +
    "<span ng-switch=\"build.status.phase\" class=\"hide-ng-leave\">\n" +
    "<span ng-switch-when=\"Failed\" class=\"status-icon\">\n" +
    "<i class=\"pficon pficon-error-circle-o text-danger\"></i>\n" +
    ...
    "</span>\n" +
    "<div ng-if=\"!overviewBuilds.hideLog && build.status.startTimestamp && build.status.phase !== 'Complete' && ('builds/log' | canI : 'get')\" class=\"animate-if\">\n" +
  ...
  $templateCache.put('views/overview/_list-row-actions.html',
    ...
    "<li ng-if=\"('buildconfigs/instantiate' | canI : 'create') && row.pipelines.length === 1\">\n" +
    "<a href=\"\" role=\"button\" ng-click=\"row.startBuild(row.pipelines[0])\">Start Pipeline</a>\n" +
    "</li>\n" +
    "<li ng-if=\"('buildconfigs/instantiate' | canI : 'create') && row.buildConfigs.length === 1 && !row.pipelines.length\">\n" +
    "<a href=\"\" role=\"button\" ng-click=\"row.startBuild(row.buildConfigs[0])\">Start Build</a>\n" +
    "</li>\n" +
    "<li ng-if=\"'deploymentconfigs' | canI : 'update'\" role=\"menuitem\">\n" +
    "<a href=\"\" role=\"button\" ng-click=\"row.startDeployment()\">Deploy</a>\n" +
    "</li>\n" +
    "<li ng-if=\"'deploymentconfigs' | canI : 'update'\" role=\"menuitem\">\n" +
  ...
  $templateCache.put('views/overview/_list-row-content.html',
    ...
    "<span ng-if=\"!row.isExpanded() && !row.hidePipelines && (row.recentPipelines | hashSize)\">\n" +
    "<build-counts builds=\"row.recentPipelines\" label=\"Pipelines\"></build-counts>\n" +
    "</span>\n" +
    "<span ng-if=\"!row.isExpanded() && (row.recentBuilds | hashSize)\">\n" +
    "<build-counts builds=\"row.recentBuilds\" label=\"Builds\"></build-counts>\n" +
    "</span>\n" +
    "</span>\n" +
    "</div>\n" +
    "</div>\n" +
    "<div ng-if=\"row.apiObject.kind === 'DeploymentConfig' && !row.current && !row.isExpanded()\" class=\"list-pf-details hidden-xs hidden-sm\">\n" +
    "<span>\n" +
    ...
    "</div>\n" +
    "</div>\n" +
    "<div ng-if=\"row.current && !row.isDeploymentInProgress() && !row.isExpanded()\" class=\"list-pf-details animate-if hidden-xs hidden-sm\">\n" +
    ...
    "<span class=\"pficon fa-fw pficon-image\"></span>\n" +
    "<image-names pod-template=\"row.apiObject\" pods=\"[row.apiObject]\"></image-names>\n" +
    "</span>\n" +
    "<span ng-if=\"row.apiObject.kind !== 'Pod' && row.current\">\n" +
    "<span class=\"pficon fa-fw pficon-image\"></span>\n" +
    "<image-names pod-template=\"row.current | podTemplate\" pods=\"row.getPods(row.current)\"></image-names>\n" +
    "</span>\n" +
    "</div>\n" +
    "</div>"
  );
  ...
  $templateCache.put('views/overview/_list-row-expanded.html',
    ...
    "<uib-tab active=\"row.selectedTab.networking\">\n" +
    "<uib-tab-heading>Networking</uib-tab-heading>\n" +
    "<overview-networking services=\"row.state.servicesByObjectUID[row.apiObject.metadata.uid]\" routes-by-service=\"row.state.routesByService\">\n" +
    "</overview-networking>\n" +
    "</uib-tab>\n" +
    ...
    "<div ng-if=\"row.state.showMetrics && !row.previous\" class=\"col-md-4 col-lg-3\">\n" +
    "<div ng-if=\"row.apiObject.kind === 'Pod'\">\n" +
    "<h4>Usage <small>Last 15 Minutes</small></h4>\n" +
    "<deployment-metrics pods=\"[row.apiObject]\" containers=\"row.apiObject.spec.containers\" profile=\"compact\" alerts=\"row.state.alerts\" class=\"overview-metrics\">\n" +
    "</deployment-metrics>\n" +
    ...
    "</div>\n" +
    "</div>\n" +
    "<div class=\"col-sm-12 col-md-5 col-lg-6\" ng-class=\"{\n" +
    "                 'col-sm-8': !row.state.showMetrics,\n" +
    "                 'hidden-sm col-md-4 col-lg-4': row.previous\n" +
    "               }\">\n" +
    "\n" +
    "<pod-template pod-template=\"row.current | podTemplate\" images-by-docker-reference=\"row.state.imagesByDockerReference\" builds=\"row.state.builds\"></pod-template>\n" +
    "</div>\n" +
    ...
    "</div>\n" +
    "\n" +
    "<overview-networking ng-if=\"!row.current || row.state.breakpoint !== 'xs'\" services=\"row.state.servicesByObjectUID[row.apiObject.metadata.uid]\" routes-by-service=\"row.state.routesByService\">\n" +
    "</overview-networking>\n" +
    "\n" +
  ...
  $templateCache.put('views/overview/_list-row.html',
    "<div class=\"list-pf-item\" ng-class=\"{ active: row.isExpanded() }\">\n" +
  ...
  $templateCache.put('views/overview/_notification-icon.html',
    ...
    "<span class=\"pficon pficon-warning-triangle-o\" dynamic-content=\"{{notification.byType.warning}}\" data-toggle=\"tooltip\" data-trigger=\"hover\" aria-hidden=\"true\"></span>\n" +
    "<span class=\"notification-icon-label\">\n" +
  ...
  $templateCache.put('views/overview/_pipelines.html',
    ...
    "<div ng-repeat=\"pipeline in overviewPipelines.recentPipelines | orderObjectsByDate : true track by (pipeline | uid)\" class=\"build-pipeline-wrapper animate-repeat animate-slide\">\n" +
    "<build-pipeline build=\"pipeline\" build-config-name-on-expanded=\"true\"></build-pipeline>\n" +
    "</div>\n" +
    "</div>"
  );
